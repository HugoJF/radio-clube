<?php if(! defined('BASEPATH'))
	...
	class Register extends CI_Controller {
		...
		public function index() {
			$this->load->view('header', array('the_user' => $this->ion_auth->user()->row()));
			$this->load->view('register_view');
			$this->load->view('footer');
		}

		public function token_check($token = -1) {
			$query = $this->db->get_where('tokens', array('value' => $token));
		...
		public function generate_token() {
			...
			while(TRUE) {
				$rand  = rand(100, 999);
				$query = $this->db->get_where('tokens', array('value' => $rand));
				if($query->num_rows() == 0)
					break;
			}
			$this->db->insert('tokens', array('value' => $rand));
			$this->load->view('header', array('the_user' => $this->ion_auth->user()->row()));
			$this->load->view('token_view', array('token' => $rand));
			$this->load->view('footer');
		}
	}
